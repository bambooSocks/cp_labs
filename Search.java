...
/**
 * ass1.Search task. No need to modify.
 */
...
public class Search {
    ...
    static char[] pattern;              // ass1.Search pattern
    ...
    static void getArguments(String[] argv) {
        ...
        try {
            ...
            System.out.print(e + "\n\nUsage:   java ass1.Search <options> file pattern [ntasks [nthreads]] \n\n"
    ...
    public static void main(String[] argv) {
        try {
            ...
            int sizeOfChunk = len / ntasks;
            int idx = 0;

            // Divide the text evenly for the tasks
            for (int i = 0; i < ntasks; i++) {
                int endIdx = idx + sizeOfChunk;

                // if end of file stop at the end
                if (endIdx >= len) {
                    endIdx = len;
                } else {
                    // loop until '|' is found and split there
                    while (text[endIdx] != '|') {
                        endIdx++;
                    }
                }
                ...
                taskList.add(new SearchTask(text, pattern, idx, endIdx));
                idx = endIdx + 1;
            }
